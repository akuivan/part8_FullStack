...
const resolvers = {
  ...
  Mutation: {
    addBook: (_, { title, author, published, genres }) => {
      ...
      let newAuthor = authors.find(a => a.name === author);

      if (!newAuthor) {
        // If not then add it
        newAuthor = { id: uuid(), name: author, born: null, bookCount: 0 };
        authors = authors.concat(newAuthor)
      }

      // Add the new book
      const newBook = { id: uuid(), title, author: author, published, genres };
